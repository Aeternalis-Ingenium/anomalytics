...
class TestQQPlotEvaluation(unittest.TestCase):
    ...
    def test_calculate_theoretical_q_pot(self):
        sorted_nonzero_ts, theoretical_q, params = calculate_theoretical_q(
            dataset=self.sample_3_ts, stats_method="POT", fit_params=self.sample_1_gpd_params  # type: ignore
        )
    ...
    def test_calculate_theoretical_q_with_invalid_series(self):
        with self.assertRaises(ValueError):
            calculate_theoretical_q(dataset="not a series", stats_method="POT", fit_params=self.sample_1_gpd_params)  # type: ignore

    def test_calculate_theoretical_q_with_unimplemented_method(self):
        with self.assertRaises(NotImplementedError):
            calculate_theoretical_q(dataset=self.sample_3_ts, stats_method="AE", fit_params=[{}])  # type: ignore

    @patch("matplotlib.pyplot.show")
    def test_visualize_qq_plot_with_valid_input(self, mock_show):
        visualize_qq_plot(dataset=self.sample_3_ts, stats_method="POT", fit_params=self.sample_1_gpd_params)  # type: ignore
        mock_show.assert_called_once()

    def test_visualize_qq_plot_with_unimplemented_method(self):
        with self.assertRaises(NotImplementedError):
            visualize_qq_plot(dataset=self.sample_3_ts, stats_method="AE", fit_params=self.sample_1_gpd_params)  # type: ignore
